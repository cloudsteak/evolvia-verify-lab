            ...
            for webapp in matching_webapps:
                ...
                if not linux_fx_version:
                    return {
                        "success": False,
                        "message": (
                            f"Web App '{webapp.name}' nem rendelkezik Linux FX "
                            f"verzióval (Docker konfiguráció hiányzik)."
                        ),
                    }
                ...
                if not (is_docker_direct or is_sitecontainers):
                    return {
                        "success": False,
                        "message": (
                            f"Web App '{webapp.name}' nem Docker konténerből fut. "
                            f"Linux FX Version: {linux_fx_version}"
                        ),
                    }

                if is_docker_direct:
                    docker_image = linux_fx_version[7:]
                    if "/" not in docker_image:
                        return {
                            "success": False,
                            "message": (
                                f"Web App '{webapp.name}' Docker image formátuma "
                                f"hibás: {docker_image}"
                            ),
                        }
